...
--- @field public data_type DataType the data type class for this attribute
--- @field public rep_change DataType (optional) the amount of change needed to trigger a report.  Only necessary for non-discrete attributes
...
function ZigbeeDriver:zigbee_message_handler(message_channel)
  ...
  if zb_rx ~= nil then
    ...
    log.trace(string.format("Received %s from %s", zb_rx, device))
...
function ZigbeeDriver.init(cls, name, driver_template)
  ...
  local original_init = out_driver.lifecycle_handlers.init or function(...) end

  -- Do we need to allow prevention of this in some way?
  out_driver.lifecycle_handlers.init = function(driver, device, event)
    original_init(driver, device, event)
    device:add_attributes_from_driver_template(driver.cluster_configurations)
    if driver.ias_zone_configuration_method ~= nil then
      device:set_ias_zone_config_method(driver.ias_zone_configuration_method)
    end
  end

  -- Make sure device is marked as configured even if a driver specific
  -- configuration is run
  local existing_configure = out_driver.lifecycle_handlers.doConfigure
  if existing_configure ~= nil then
    out_driver.lifecycle_handlers.doConfigure = function(driver, device)
      existing_configure(driver, device)
      device:set_field(constants.ZIGBEE_DEVICE_CONFIGURED_KEY, true, {persist = true})
    end
  end
  ...
  device_management.init_device_config(out_driver)
