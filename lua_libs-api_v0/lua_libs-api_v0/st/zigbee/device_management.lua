...
---
--- @param device ZigbeeDevice the device to address the read attribute to
...
--- @return ReadAttribute the constructed read attribute ZigbeeTx command
...
---
--- @param device Device the device to address the bind request to
--- @param cluster number the cluster id of the attribute to read
--- @param hub_zigbee_eui string the byte string for the hub's zigbee eui for binding
--- @return BindRequest the constructed bind request ZigbeeTx command
...
--- separately.
---
--- @param device Device the device to configure for reporting
--- @param attr_config AttributeConfiguration the attribute configuration to set up
--- @return ConfigureReporting the constructed configure reporting ZigbeeTx command
...
---
--- @param device Device the device to write the CIE address to
...
---
--- @param device Device the device to configure IAS Zone for
...
function device_management.check_all_device_config(driver)
  local dev_list = driver.device_api.get_device_list()
  for _, dev_id in ipairs(dev_list) do
    local device = driver:get_device_info(dev_id)
    local config_completed = device:get_field(constants.ZIGBEE_DEVICE_CONFIGURED_KEY) or false
    if not config_completed then
      device.thread:queue_event(driver.lifecycle_dispatcher.dispatch, driver.lifecycle_dispatcher, driver, device, "doConfigure")
    end
  end
end
...
---
--- @param driver Driver the driver to configure health checks for
function device_management.init_device_config(driver)
  local jitter = math.random(0, 5)
  driver:call_with_delay(5 + jitter, device_management.check_all_device_config, "device config setup")
end
