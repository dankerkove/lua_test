...
--- @field public src_address Uint16 the source address to bind to
...
--- @return BindRequest the parsed attribute record
...
--- @function BindRequest:serialize
--- @return string this BindRequest serialized
BindRequest.serialize = utils.serialize_from_fields
...
--- @param src_address Uint16 the source address to bind to
--- @param src_endpoint Uint8 the source endpoint to bind to
--- @param cluster_id ClusterId the cluster to bind
--- @param dest_addr_mode Uint8 number a field describing which addressing method will be used for the destination address
--- @param dest_address Uint16 or IeeeAddress this will be Uin16 if dest_addr_mode is 0x01, or IeeeAddress if it is 0x03
--- @param dest_endpoint Uint8 optional only necessary if dest_addr_mode is 0x03 for long addresses
--- @return BindRequest the constructed BindRequest
function BindRequest.from_values(orig, src_address, src_endpoint, cluster_id, dest_addr_mode, dest_address, dest_endpoint)
  ...
  elseif out.dest_addr_mode.value == bind_request.ADDRESS_MODE_64_BIT then
    ...
    error("Unrecognized destination address mode: " .. out.dest_addr_mode.value)
  end
