...
--- @field public args table<string, _> the arguments for this function in a key, value form
--- @field public positional_args _[] The arguments in positional format
...
capabilities.build_cap_from_json_string = function(cap_json)
  ...
  local cap = capabilities.build_capability(cap_raw_tab.name:gsub(" ", ""), cap_raw_tab.id)
...
cap_mt.__index = function(self, key)
  ...
  elseif cap_mt.__key_cache[key] == nil then
    -- TODO: load this JSON cap def from RPC when available
    local req_path = string.format("st.capabilities.generated.%s", key)
    cap_mt.__key_cache[key] = capabilities.build_cap_from_json_string(require(req_path))
  end
  return cap_mt.__key_cache[key]
end
...
capability_utils.build_capability = function(capability_name, capability_id)
  local capability = {}
...
capability_utils.emit_event = function(device, component_id, sock, event)
  ...
  if is_valid then
    ...
    log.info(string.format("%s emitting event: %s", device, json.encode(edge_device_event)))
    ...
    log.warn("Received invalid value: " .. utils.stringifyTable(event.value) .. " for " .. event.capability.NAME .. "." .. event.attribute.NAME)
    return nil
  end
end
