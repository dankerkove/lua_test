...
local function get_refresh_commands(self, device)
  if device:supports_capability_by_id(capabilities.carbonMonoxideDetector.ID) then
    if device:is_cc_supported(cc.NOTIFICATION) then
      return {Notification:Get({ notification_type = Notification.notification_type.CO })}
...
--- @alias carbon_monoxide_detector_defaults
