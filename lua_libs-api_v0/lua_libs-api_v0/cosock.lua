...
-- temp reset cosock for tests
function m.reset()
  threads = {} --TODO: use set instead of list
  threadnames = setmetatable({}, weakkeys)
  threadswaitingfor = {} -- what each thread is waiting for
  readythreads = {} -- like wakethreads, but for next loop (can be modified while looping wakethreads)
  socketwrappermap = setmetatable({}, weaktable) -- from native socket to async socket
  threaderrorhandler = nil -- TODO: allow setting error handler
  timers = {}
end
