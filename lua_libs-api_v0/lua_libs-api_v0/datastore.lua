...
local function check_if_valid(value)
    ...
    if t == "table" then
        ...
        if table_mt ~= nil and not our_mts[table_mt] then
            error("datastore table values should be simple and not include metatable functionality")
        end
    ...
    error("Data store keys and values must be JSON encodable: " .. tostring(value) .. " is of unsupported type " .. type(value))
end
...
local function new_mt(parent)
    ...
    mt.__newindex = function(self, key, value)
        if mt.__funcs[key] ~= nil then
            error("Key: " .. key .. " refers to a protected method, do not overwrite.")
        end
    ...
    if parent == nil then
        ...
        function mt.__funcs:save()
            ...
            if succ then
                ...
                error("Unable to serialize datastore value: " .. val)
            end
        end
    end
